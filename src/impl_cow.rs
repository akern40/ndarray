...
where D: Dimension
{
    fn from(view: ArrayView<'a, A, D>) -> CowArray<'a, A, D>
    {
        ...
        unsafe { ArrayBase::from_data_ptr(CowRepr::View(view.data), view.meta.ptr).with_strides_dim(view.meta.strides, view.meta.dim) }
    }
}
...
where D: Dimension
{
    fn from(array: Array<A, D>) -> CowArray<'a, A, D>
    {
        // safe because equivalent data
        unsafe {
            ArrayBase::from_data_ptr(CowRepr::Owned(array.data), array.meta.ptr).with_strides_dim(array.meta.strides, array.meta.dim)
        }
    }
}
