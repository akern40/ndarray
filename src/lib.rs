...
//    `meta.ptr` must point into the data represented by `data`, and the
//    `meta.dim` and `meta.strides` must be consistent with `data`. For example,
//
//    * If `data` is `OwnedRepr<A>`, all elements represented by `meta.ptr`,
//      `meta.dim`, and `meta.strides` must be owned by the `Vec` and not aliased
//      by multiple indices.
//
//    * If `data` is `ViewRepr<&'a mut A>`, all elements represented by `meta.ptr`,
//      `meta.dim`, and `meta.strides` must be exclusively borrowed and not aliased by
//      multiple indices.
//
// 2. If the type of `data` implements `Data`, then `meta.ptr` must be aligned.
//
// 3. `meta.ptr` must be non-null, and it must be safe to [`.offset()`] `meta.ptr` by
//    zero.
//
...
//    axes, so in addition for it being safe to offset `meta.ptr` by each of these
...
//      type is zero-sized. In these cases, `meta.ptr` may be dangling, but it must
...
//    the offsets of interest relative to `meta.ptr` are -2, -1, 0, 1, 2, 3. So,
//    `meta.ptr.offset(-2)`, `meta.ptr.offset(-1)`, …, `meta.ptr.offset(3)`
//    must be pointers within a single allocation with element type `A`; `(3 - (-2)) *
...
where S: RawData
{
    /// Array metadata: shape, strides, and pointer
...
mod array_ref
{
    ...
    /// (if it exists) can be written as
    /// ```
    /// fn<A, D> first_elem(arr: &ArrayRef<A, D>) -> Option<A> {
    ///     arr.get(0)
    /// }
    /// ```
    ///
