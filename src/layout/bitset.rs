...
#[derive(Copy, Clone)]
pub struct Bitset(pub(super) u32);
...
impl Bitset
{
    ...
    #[inline(always)]
    pub(crate) fn intersect(self, other: Bitset) -> Bitset
    {
    ...
    #[inline(always)]
    pub(crate) fn also(self, other: Bitset) -> Bitset
    {
        Bitset(self.0 | other.0)
    }

    #[inline(always)]
    pub(crate) fn one_dimensional() -> Bitset
    {
        Bitset::c().also(Bitset::f())
    }

    #[inline(always)]
    pub(crate) fn c() -> Bitset
    {
        Bitset(Bitset::CORDER | Bitset::CPREFER)
    }

    #[inline(always)]
    pub(crate) fn f() -> Bitset
    {
        Bitset(Bitset::FORDER | Bitset::FPREFER)
    }

    #[inline(always)]
    pub(crate) fn cpref() -> Bitset
    {
        Bitset(Bitset::CPREFER)
    }

    #[inline(always)]
    pub(crate) fn fpref() -> Bitset
    {
        Bitset(Bitset::FPREFER)
    }
    ...
    {
        (self.is(Bitset::CORDER) as i32 - self.is(Bitset::FORDER) as i32)
            + (self.is(Bitset::CPREFER) as i32 - self.is(Bitset::FPREFER) as i32)
    }
}
...
mod tests
{
    ...
    fn contig_layouts()
    {
        ...
        assert!(ac.is(Bitset::CORDER) && ac.is(Bitset::CPREFER));
        assert!(!ac.is(Bitset::FORDER) && !ac.is(Bitset::FPREFER));
        assert!(!af.is(Bitset::CORDER) && !af.is(Bitset::CPREFER));
        assert!(af.is(Bitset::FORDER) && af.is(Bitset::FPREFER));
    }
    ...
    fn stride_layouts()
    {
...

        {
            ...
            assert!(v1.is(Bitset::CORDER) && v1.is(Bitset::CPREFER));
            assert!(!v1.is(Bitset::FORDER) && !v1.is(Bitset::FPREFER));
            assert!(!v2.is(Bitset::CORDER) && v2.is(Bitset::CPREFER));
            assert!(!v2.is(Bitset::FORDER) && !v2.is(Bitset::FPREFER));
        }
